...
########################################################################################################
### Acknowledgement                                                                                  ###
### Arjen Deetman (http://arjendeetman.nl/)                                                          ###
### Origin code from https://github.com/arjendeetman/TopOptAbaqus                                    ###
########################################################################################################
...
processName='TEST-OPT-2D-MODEL-50' ## Process name for topology optimization
...
minSizeLeftRight=float(meshSize)
...
## Cycles
mDC=int(50) ## Maximum number of cycles

# Computer settings
numCores=int(4) # Number of cores that will be uese of the processor
perMem=int(50) # Percentage of the memory that will be uses of the computer in %
...
if minSizeLeftRight>0:
    ...
    modske=mod.sketches['__profile__']
    modpa.projectReferencesOntoSketch(filter=COPLANAR_EDGES, sketch=modske)
    mod.sketches['__profile__'].Line(point1=(-x+minSizeLeftRight, -y), point2=(-x+minSizeLeftRight, y-minSizeTop))
    modpa.PartitionFaceBySketch(faces=modpa.faces.findAt(((0.0, 0.0, 0.0), )), sketch=modske)
    del modske, mod.sketches['__profile__']
    modra.regenerate()
...
## Right layer
findatRight=(x-float(minSizeLeftRight)/2, 0.0, 0.0)
modra.Set(faces=modrain.faces.findAt((findatRight, )), name='RIGHT-LAYER')
## Left layer
findatLeft=(-x+float(minSizeLeftRight)/2, 0.0, 0.0)
modra.Set(faces=modrain.faces.findAt((findatLeft, )), name='LEFT-LAYER')
## Design area
findat1=(0.0, y-float(minSizeTop)/2, 0.0)
findat2=(0.0, y-minSizeTop-float(y*2-minSizeTop)/2, 0.0)
modra.Set(faces=modrain.faces.findAt((findat1, ), (findat2, ), (findatLeft, ), (findatRight, )), name='DESIGN-AREA')
del findat, findat1, findat2, findatLeft, findatRight
...
## RP-2
mod.DisplacementBC(createStepName='Initial', name='BC-RP-2', region=modra.sets['RP-2'], u1=SET, u2=SET)
...
if minSizeLeftRight>0:
    modopt.FrozenArea(name='FROZEN-AREA-LEFT-LAYER', region=modra.sets['LEFT-LAYER'])
    modopt.FrozenArea(name='FROZEN-AREA-RIGHT-LAYER', region=modra.sets['RIGHT-LAYER'])
## Minimum member size
minSize=2*rmin
if minSize>0:
